...
def main():
    ...
    parser.add_option('-b', dest='batch_size', default=100, type='int', help='Batch size (affects memory usage) [Default: %default]')
    ...
    parser.add_option('-m', dest='motifs_file', default = '/home/fangzj/Workdir/Basset_Unblocked/scripts/optuna/weights/m1d_optadam_drop0.33_lr0.0005_step15.00_gama0.49_mom0.93_run_0/m1d_optadam_drop0.33_lr0.0005_step15.00_gama0.49_mom0.93_run_0/motifsout/table.txt')
    ...
    parser.add_option('-o', dest='out_dir', default='./infl')
    parser.add_option('--subset', dest='subset_file', default='/home/fangzj/Workdir/Basset_Unblocked/scripts/project/infl/usefuldata/primary_cells.txt', help='Subset targets to the ones in this file')
    ...
    parser.add_option('-t', dest='targets_file', default="/home/fangzj/Workdir/Basset_Unblocked/scripts/project/infl/usefuldata/cell_activity.txt", help='File specifying target indexes and labels in table format')
    ...
    if options.model_hdf5_file is None:
        ...
        options.model_hdf5_file = '{}/model_out.h5'.format(options.out_dir)
        pytorch_cmd = 'python {}basset_motifs_null.py --model_file {} --data_file {} --out_file {} ' \
                      '--batch_size {}'.format("/home/fangzj/Workdir/Basset_Unblocked/scripts/project/",
                                               model_file,test_hdf5_file, options.model_hdf5_file,options.batch_size)
        subprocess.call(pytorch_cmd, shell=True)
    ...
    if options.norm_targets:
        ...
        for fi in range(len(filter_infl)):
            ...
            tnorm_out.write('{:3d} {:7.4f} {:7.4f}'.format(*cols))
        tnorm_out.close()
    ...
    for i in range(len(filter_infl)):
        if df_motifs is not None:
            ...
            print(f'{cols[0]:3d} {cols[1]:7.4f} {cols[2]:6.4f} {cols[3]:6.3f} {cols[4]:4.1f} {cols[5]:8s}', file= table_out)
        else:
            cols = (i, filter_infl[i], filter_means[i], filter_stds[i])
            print(f'{cols[0]:3d} {cols[1]:7.4f} {cols[2]:6.4f} {cols[3]:6.3f} ', file=table_out)
    table_out.close()
    ...
    if options.motifs_file:
        ...
        if not options.color_filters:
            g = sns.jointplot(x=np.array(df_motifs.ic.loc[nonzero]), y=filter_infl[nonzero], color='black',  joint_kws={'alpha':0.8})
        else:
            g = sns.jointplot(x=np.array(df_motifs.ic.loc[nonzero]), y=filter_infl[nonzero], color='black',  joint_kws={'alpha':0.1})
    ...
    for i in range(df_ft.shape[0]):
        for ti in range(len(target_names)):
            ...
            print(f'{cols[0]:-3d} {cols[1]:3d} {cols[2]:20s} {cols[3]:7.4f}', file=table_out)
    table_out.close()
        ...
        for si in range(seq_filter_targets.shape[0]):
            for fi in range(seq_filter_targets.shape[1]):
                for ti in range(seq_filter_targets.shape[2]):
                    ...
                    print(f'{cols[0]:-25s} {cols[1]:3d} {cols[2]:3d} {cols[3]:7.4f}', file=table_out)
        table_out.close()

    # use only high information filters
    if options.informative_only and df_motifs is not None:
        df_ft = df_ft[df_motifs.ic > 6]
...
def plot_infl_heatmaps(unit_target_deltas, out_dir, width=10, height=20, font=0.4, target_colors=None):
    ...
    unit_target_deltas_norm.to_csv("{}/unit_target_deltas_norm.csv".format(out_dir), index=True, header=True)
